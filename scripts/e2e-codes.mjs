...
async function main() {
  ...
  r = await api('POST', '/cash/advances', {
    token: adminToken,
    body: { tellerId: teller.id, collectorCode: newCollector.code, amount: 1000 }
  })
  ...
  r = await api('POST', '/cash/remits', {
    token: tellerToken,
    body: { collectorCode: newCollector.code, amount: 1000 }
  })
  ...
  if (otherTeller) {
    ...
    const advForOther = await api('POST', '/cash/advances', {
      token: adminToken,
      body: { tellerId: otherTeller.id, collectorCode: newCollector.code, amount: 50 }
    })
  ...
  if (openFight) {
    ...
    await api('POST', '/cash/advances', {
      token: adminToken,
      body: { tellerId: teller.id, collectorCode: newCollector.code, amount: 100 }
    })
